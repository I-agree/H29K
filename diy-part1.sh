...
# =============== BEGIN: Apply I-agree's official 999-fix-early-init-dt-scan-for-6.12.patch ===============
echo "🔧 Applying upstream-maintained patch from https://github.com/I-agree/H29K..."

cd /workdir/openwrt || { echo "❌ Failed to enter openwrt dir"; exit 1; }

# Ensure rockchip feed is present and up-to-date
if [ ! -d "feeds/rockchip" ]; then
    echo "⚠️  rockchip feed not found. Updating feeds..."
    ./scripts/feeds update -a 2>/dev/null || true
fi

# Switch to rockchip feed directory
cd feeds/rockchip || { echo "❌ Failed to enter feeds/rockchip"; exit 1; }

# Create patches dir for kernel 6.12
mkdir -p patches-6.12

# Download and apply patch directly (no intermediate file)
# Using GitHub raw URL — guaranteed stable if blob SHA stays same
PATCH_URL="https://raw.githubusercontent.com/I-agree/H29K/main/999-fix-early-init-dt-scan-for-6.12.patch"

# Retry logic (GitHub raw sometimes 503s in Actions)
for i in {1..3}; do
    if curl -sL "$PATCH_URL" | patch -p1 -d . -o "patches-6.12/999-fix-early-init-dt-scan-for-6.12.patch" 2>/dev/null; then
        echo "✅ Patch applied successfully (attempt $i)"
        break
    else
        echo "⚠️  Patch apply failed (attempt $i), retrying..."
        sleep 2
    fi
    if [ $i -eq 3 ]; then
        echo "❌ Failed to apply patch after 3 attempts. Check URL or network."
        exit 1
    fi
done

# Verify patch was written and contains expected symbol
if [ ! -s "patches-6.12/999-fix-early-init-dt-scan-for-6.12.patch" ]; then
    echo "❌ Patch file is empty"
    exit 1
fi

if ! grep -q "early_init_dt_verify" "patches-6.12/999-fix-early-init-dt-scan-for-6.12.patch"; then
    echo "❌ Patch does not contain 'early_init_dt_verify' — malformed?"
    exit 1
fi

echo "✅ Patch saved to feeds/rockchip/patches-6.12/999-fix-early-init-dt-scan-for-6.12.patch"
# =============== END: Apply patch ===============
