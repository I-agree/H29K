...
sed -i '/CONFIG_DEFAULT_NET_CONG/d' "$CONFIG_FILE"
sed -i '/CONFIG_DEFAULT_BBR/d' "$CONFIG_FILE"

# B. 一次性注入完全适配主线 Linux 6.12.91 的 H29K 内核技术栈
cat >> "$CONFIG_FILE" << 'EOF'
...
# --- 采用现代 Schedutil 智能调度模式（兼顾爆发性能与日常待机散热） ---
...
# === 6. 写入完整独立编译 Override 规则（🌟 核心修复：彻底改为自适应相对路径） ===
