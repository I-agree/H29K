...
# --- 2. 硬件支持文件下载与强制断言 ---
# 下载 H29K 专属 DTS 和引导程序，下载失败则立即停止编译，防止产生坏件
...
# --- 3. 动态注入 video.mk (仅保留 ST7789V，使用 AutoLoad,09 参数) ---
# 使用通配符寻找 .ko，确保在不同版本的内核中都能正确打包，无需核心依赖包
...
# --- 4. 自动修补所有内核配置 (开启全硬件支持、BBR 及驱动依赖) ---
# 强制开启核心依赖，锁定 BBR 默认值以防编译环境交互提问导致错误
echo "正在修补内核 config (开启 BBR/STAGING/ST7789V/5G)..."
...
# --- 5. 注册设备并完善 DEVICE_PACKAGES (按需调整 aic8800-sdio 与 qmodem) ---
# 集成板载有线/无线、5G模块、屏幕驱动、中断均衡及拨号工具
TARGET_MK=$(find target/linux/rockchip/image -name "armv8.mk")
if [ -n "$TARGET_MK" ] && ! grep -q "hinlink_h29k" "$TARGET_MK"; then
    echo "正在向 $TARGET_MK 注册 H29K 设备包并调整驱动包..."
    cat >> "$TARGET_MK" <<EOF
