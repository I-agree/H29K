...
# 🌟【核心修复 1】末尾加上 || true。防止 GitHub Actions 共享 IP 触发 API 限流时导致 grep 失败，进而在 pipefail 模式下让整个脚本直接猝死
...
# 🌟【核心修复 2】加上 2>/dev/null 和 || true，防止 Docker 提取版本号失败时直接导致 set -e 熔断编译
